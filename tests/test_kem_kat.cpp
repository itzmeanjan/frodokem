...
// Test if
//
// - Is Frodo1344 KEM implemented correctly ?
// - Is it conformant with the specification ?
//
// using KATs.
TEST(FrodoKEM, Frodo1344KEMKAT)
{
  using namespace std::literals;
  namespace utils = frodo_utils;

  const std::string kat_file = "./kats/FrodoKEM1344_KAT.txt";
  std::fstream file(kat_file);
  ...
  while (true) {
    ...
    if (!std::getline(file, s).eof()) {
      ...
      auto _s = std::string_view(s);
      auto __s = _s.substr(_s.find("="sv) + 2, _s.size());
      auto ___s = utils::from_hex<frodo1344_kem::len_sec / 8>(__s);

      auto _seedSE = std::string_view(seedSE);
      auto __seedSE = _seedSE.substr(_seedSE.find("="sv) + 2, _seedSE.size());
      auto ___seedSE = utils::from_hex<frodo1344_kem::len_SE / 8>(__seedSE);

      auto _z = std::string_view(z);
      auto __z = _z.substr(_z.find("="sv) + 2, _z.size());
      auto ___z = utils::from_hex<frodo1344_kem::len_A / 8>(__z);
      ...
      auto _μ = std::string_view(μ);
      auto __μ = _μ.substr(_μ.find("="sv) + 2, _μ.size());
      auto ___μ = utils::from_hex<frodo1344_kem::len_sec / 8>(__μ);

      auto _salt = std::string_view(salt);
      auto __salt = _salt.substr(_salt.find("="sv) + 2, _salt.size());
      auto ___salt = utils::from_hex<frodo1344_kem::len_salt / 8>(__salt);

      auto _ct = std::string_view(ct);
      auto __ct = _ct.substr(_ct.find("="sv) + 2, _ct.size());
      auto ___ct = utils::from_hex<frodo1344_kem::CIPHER_LEN>(__ct);

      auto _ss = std::string_view(ss);
      auto __ss = _ss.substr(_ss.find("="sv) + 2, _ss.size());
      auto ___ss = utils::from_hex<frodo1344_kem::len_sec / 8>(__ss);

      std::array<uint8_t, frodo1344_kem::PUB_KEY_LEN> pubkey{};
      std::array<uint8_t, frodo1344_kem::SEC_KEY_LEN> seckey{};
      std::array<uint8_t, frodo1344_kem::CIPHER_LEN> ctxt{};
      std::array<uint8_t, frodo1344_kem::len_sec / 8> shrd_sec0{};
      std::array<uint8_t, frodo1344_kem::len_sec / 8> shrd_sec1{};

      frodo1344_kem::keygen(___s, ___seedSE, ___z, pubkey, seckey);
      frodo1344_kem::encaps(___μ, ___salt, pubkey, ctxt, shrd_sec0);
