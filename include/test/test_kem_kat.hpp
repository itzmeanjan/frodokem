...
test_frodo640_kem_kat()
{
  ...
  while (true) {
    ...
    if (!std::getline(file, s).eof()) {
      ...
      // auto ___skey = utils::from_hex<frodo640_kem::SEC_KEY_LEN +
      // 640>(__skey);
...
test_frodo976_kem_kat()
{
  ...
  while (true) {
    ...
    if (!std::getline(file, s).eof()) {
      ...
      frodo976_kem::encaps(___μ, pubkey, ctxt, shrd_sec0);
...
test_frodo1344_kem_kat()
{
  ...
  while (true) {
    ...
    if (!std::getline(file, s).eof()) {
      ...
      frodo1344_kem::encaps(___μ, pubkey, ctxt, shrd_sec0);
