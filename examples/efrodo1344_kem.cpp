...
// sha3/include -I subtle/include examples/efrodo1344_kem.cpp
int
main()
{
  ...
  std::vector<uint8_t> pkey(efrodo1344_kem::PUB_KEY_LEN, 0);
  std::vector<uint8_t> skey(efrodo1344_kem::SEC_KEY_LEN, 0);
  std::vector<uint8_t> μ(μ_LEN, 0);
  std::vector<uint8_t> ss0(SS_LEN, 0);
  std::vector<uint8_t> cipher(efrodo1344_kem::CIPHER_LEN, 0);
  ...
  std::span<uint8_t, efrodo1344_kem::PUB_KEY_LEN> _pkey{ pkey };
  std::span<uint8_t, efrodo1344_kem::SEC_KEY_LEN> _skey{ skey };
  std::span<uint8_t, μ_LEN> _μ{ μ };
  std::span<uint8_t, SS_LEN> _ss0{ ss0 };
  std::span<uint8_t, efrodo1344_kem::CIPHER_LEN> _cipher{ cipher };
  ...
  efrodo1344_kem::keygen(_s, _seedSE, _z, _pkey, _skey);
  efrodo1344_kem::encaps(_μ, _pkey, _cipher, _ss0);
  efrodo1344_kem::decaps(_skey, _cipher, _ss1);
